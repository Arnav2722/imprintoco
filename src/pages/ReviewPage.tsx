...
const ReviewPage = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!imageFile) return alert("Upload a photo of your setup.");
    setSubmitting(true);

    try {
      // Note: Replace with your actual Cloudinary credentials
      const data = new FormData();
      data.append("file", imageFile);
      data.append("upload_preset", "your_unsigned_preset");
  ...
  return (
    <div className="min-h-screen bg-background text-foreground font-body">
      {/* The main Navbar is handled globally in App.tsx. 
          The padding-top (pt-32) below ensures content doesn't sit under the Navbar.
      */}

      <main className="max-w-[1400px] mx-auto pt-32 md:pt-40 pb-20 px-6 sm:px-12">
        {/* ULTRA-RESPONSIVE BACK BUTTON */}
        <button
          onClick={() => navigate("/")}
          className="relative z-[100] flex items-center gap-2 group mb-10 text-foreground/40 hover:text-primary transition-colors cursor-pointer pointer-events-auto"
        >
          <ArrowLeft
            size={18}
            className="group-hover:-translate-x-1 transition-transform"
                  ...
                  <div className="flex gap-2">
                    {[1, 2, 3, 4, 5].map((num) => (
                      <button
                        ...
                        className={`flex-1 h-12 border-2 border-foreground font-black transition-colors ${formData.rating >= num ? "bg-primary text-black" : "bg-transparent text-foreground/30"}`}
                      >
                        {num}
                      </button>
                    ))}
