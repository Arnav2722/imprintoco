...
const faqs = [
  {
    question: "Bulk ordering for outlets?",
    answer:
      "Yes. We cater to bulk requirements for businesses and commercial outlets. Drop us a message on WhatsApp for custom rates and logistical assistance.",
    icon: Box,
  },
  {
    question: "Order custom designs?",
    answer:
      "Simple. Navigate to the 'Custom' tab in our menu. Upload your high-resolution artwork, select your size, and we will handle the precision printing.",
    icon: Paintbrush,
  },
  {
  ...
  {
    question: "How to track status?",
    answer:
      "Every order is tracked. Once secured, you will receive automated WhatsApp and Email notifications with your unique tracking ID and real-time updates.",
    icon: MapPin,
  },
];
...
const FAQSection = () => {
  ...
  return (
    <section className="py-12 md:py-32 bg-background selection:bg-primary selection:text-black">
      <div className="max-w-[1000px] mx-auto px-6 md:px-10">
        ...
        <div className="mb-10 md:mb-20 text-center">
          ...
          >
            ...
            <span className="text-[7px] md:text-[10px] font-black uppercase tracking-[0.2em] text-foreground/40">
              Information Hub
            </span>
          </motion.div>
          <h2 className="font-display text-[10vw] sm:text-6xl md:text-8xl font-black tracking-tighter uppercase leading-none text-foreground">
            FAQ
          </h2>
        </div>
            ...
            return (
              <motion.div
                ...
                className={`border-2 transition-all duration-300 ${
                  isOpen
                    ? "border-foreground bg-white shadow-[6px_6px_0px_0px_rgba(0,212,255,1)]"
                    : "border-foreground/10 bg-white/50 hover:border-foreground/30 shadow-none"
                }`}
              >
                <button
                  onClick={() => setOpenIndex(isOpen ? null : i)}
                  className="w-full p-5 md:p-10 flex items-center justify-between text-left group"
                >
                  <div className="flex items-center gap-4 md:gap-8">
                    <div
                    ...
                    <span className="font-display text-xs md:text-2xl font-black uppercase tracking-tight text-foreground leading-[1.2]">
                      {faq.question}
                    </span>
                  </div>
                  <div className="ml-4 flex-shrink-0">
                    <div
                      className={`w-8 h-8 md:w-12 md:h-12 flex items-center justify-center border-2 border-foreground transition-all ${
                        isOpen ? "bg-primary" : "bg-transparent"
                      }`}
                    >
                      {isOpen ? (
                        <Minus
                          className="w-4 h-4 md:w-6 md:h-6 text-foreground"
                          strokeWidth={3}
                        />
                      ) : (
                        <Plus
                          className="w-4 h-4 md:w-6 md:h-6 text-foreground/20 group-hover:text-foreground transition-colors"
                          strokeWidth={3}
                        />
                      )}
                    </div>
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className="px-5 md:px-10 pb-8 md:pb-10">
                        <div className="ml-0 md:ml-14 pl-4 md:pl-8 border-l-2 md:border-l-4 border-accent">
                          <p className="text-foreground/60 text-[9px] md:text-lg font-black uppercase leading-relaxed max-w-2xl">
                            {faq.answer}
                          </p>
                        </div>
                      </div>
        ...
        <div className="mt-12 md:mt-24 text-center">
          <div className="p-6 md:p-10 bg-accent-lime border-2 md:border-4 border-foreground shadow-[6px_6px_0px_0px_rgba(0,0,0,1)] inline-block w-full sm:w-auto">
            <p className="text-[8px] md:text-xs font-black text-foreground uppercase tracking-widest mb-4">
              Need direct assistance?
            </p>
            <a
              href="/contact"
              className="font-display text-base md:text-2xl font-black uppercase tracking-tighter text-foreground border-b-4 border-foreground hover:text-primary hover:border-primary transition-all inline-block"
            >
              Contact Support
            </a>
          </div>
